...
class Admin_model extends CI_Model
{
   ...
   public function get_mock_detail($id)
    ...
    public function save_message($folder = 'draft', $sender = '', $sender_email = '', $send_to = '')
    {
        ...
        if ($sender_email == '') {
            $info['sender_email'] = $this->db->select('support_email')->where('brand_id', 1)->get('settings')->support_email;
        } else {
            $info['sender_email'] = $sender_email;
        }
        if ($send_to == '') {
            $info['message_send_to'] = $this->db->select('support_email')->where('brand_id', 1)->get('settings')->support_email;
        } else {
